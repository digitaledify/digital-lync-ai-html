...
const WhyJavaTraining = ({ title, headerTitle }: { title: string, headerTitle: string[] |undefined }) => {
    console.log("🚀 ~ WhyJavaTraining ~ headerTitle:", headerTitle)

    const data: CareerDataItem[] = [
        {
            id: 1,
    ...
    const [activeTab, setActiveTab] = useState<CareerDataItem | undefined>(data[0]); // Default to the first tab
    ...
    return (
        <div className="container px-2 mx-auto mt-20 font-poppins">
            ...
            <div className="flex-row md:flex items-center mx-auto w-fit md:w-full justify-center md:justify-between md:border-b border-[#DDDDDD] mb-12">
                {headerTitle && headerTitle?.map((item, index) => {
                    const selectedData: CareerDataItem | undefined = data.find((d) => d.id === index + 1);
                    return (
                        <p
                            key={index + 1}
                            onClick={() => setActiveTab(selectedData)}
